...
public interface ICertPropReaderD {
    ...
    /// <exception cref="ArgumentException">CA is not configured to maintain Delta CRLs.</exception>
    ...
    /// <summary>
    /// Gets an ordered collection of X.520 relative distinguished name (RDN) attributes used by Certification Authority to order attributes in certificate subject.
    /// </summary>
    /// <returns>An ordered collection of X.520 RDN attributes.</returns>
    OidCollection GetSubjectTemplateOIDs();
    /// <summary>
    /// Gets a number of configured partitions in partitioned CRL.
    /// </summary>
    /// <returns>A number of configured partitions in partitioned CRL</returns>
    Int32 GetPartitionCount();
    /// <summary>
    /// Gets a Base CRL for a specific Certification Authority signing key and CRL partition.
    /// </summary>
    /// <param name="caKeyIndex">Zero-based CA key index.</param>
    /// <param name="partitionIndex">Zero-based CRL partition index.</param>
    /// <returns>ASN.1 encoded CRL.</returns>
    Byte[] GetPartitionedBaseCrl(Int32 caKeyIndex, Int32 partitionIndex);
    /// <summary>
    /// Gets a Delta CRL for a specific Certification Authority signing key and CRL partition.
    /// </summary>
    /// <param name="caKeyIndex">Zero-based CA key index.</param>
    /// <param name="partitionIndex">Zero-based CRL partition index.</param>
    /// <returns>ASN.1 encoded CRL.</returns>
    /// <exception cref="ArgumentException">CA is not configured to maintain Delta CRLs.</exception>
    Byte[] GetPartitionedDeltaCrl(Int32 caKeyIndex, Int32 partitionIndex);
    /// <summary>
    /// Gets a Base CRL publish status for a specific Certification Authority signing key and CRL partition.
    /// </summary>
    /// <param name="caKeyIndex">Zero-based CA key index.</param>
    /// <param name="partitionIndex">Zero-based CRL partition index.</param>
    /// <returns>Requested CRL publish status.</returns>
    AdcsPropCrlPublishState GetPartitionedBaseCrlPublishStatus(Int32 caKeyIndex, Int32 partitionIndex);
    /// <summary>
    /// Gets a Delta CRL publish status for a specific Certification Authority signing key and CRL partition.
    /// </summary>
    /// <param name="caKeyIndex">Zero-based CA key index.</param>
    /// <param name="partitionIndex">Zero-based CRL partition index.</param>
    /// <returns>Requested CRL publish status.</returns>
    /// <exception cref="ArgumentException">CA is not configured to maintain Delta CRLs.</exception>
    AdcsPropCrlPublishState GetPartitionedDeltaCrlPublishStatus(Int32 caKeyIndex, Int32 partitionIndex);
}